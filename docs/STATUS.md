...
| Phase 1 · Guided Companion | Daily logging, insights, and JSON export exist, but PDF outputs, sharing, analytics, and native packaging are outstanding. | 28% | CSV log schema and download flow are live; next deliverables are richer share UX, instrumentation, and Capacitor shells. |
| Phase 2 · Connected Care | Not started. | 0% | Requires backend selection, auth, sync, and clinician dashboard. |
| Cross-cutting workstreams | Documentation, tests, and design system seeds exist; automated quality and compliance tracks are emerging. | 40% | Added plan logic tests for insights/export; next up are CI jobs, accessibility backlog, and security checklist. |
...
4. Extend the export spec to cover the PDF clinician packet—CSV contract now documented in `docs/EXPORTS.md`.
